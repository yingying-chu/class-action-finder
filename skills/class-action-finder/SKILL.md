...
- In Filed & tracking, clearly separate awaiting-payout and paid claims. Show filed date, claim ID/PIN, expected payout, actual payout, payment method, and current status so the report preserves the user’s claim history. For a filed claim whose filing window is still open, show `Open window` and its deadline here rather than duplicating the card in Active claims.
