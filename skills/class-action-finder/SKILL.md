...
- In Filed & tracking, clearly separate awaiting-payout and paid claims. Show filed date, claim ID/PIN, expected payout, actual payout, payment method, and current status so the report preserves the user’s claim history.
...
- Render the five lifecycle headings as semantic names without `Section 1–5` numerals: `Active claims`, `Watching`, `Expired`, `Filed & tracking`, and `Security alerts`. The action queue is the single owner of complete details for unfiled 🟢/🟡 filing actions. Active claims contains only additional open items, such as already-filed, auto-enrolled, or 🟠 cases, so actionable cards are not duplicated. Keep all five anchors present even when empty; collapse an empty Active claims section to one quiet line. Sort nonempty Active claims by soonest deadline. An active claim that is already filed appears there with its filed badge and in Filed & tracking, but never in the action queue.
...
2. If a report exists, offer: *"Want me to update your latest report to show this?"* If yes:
   ...
   - Add or update the corresponding Filed & tracking card and increment the Filed count only if the claim was not already counted there. Update the status-strip counts, reduce Action required if the claim was previously actionable, and recalculate the hero's actionable potential value. If the claim window remains open, move its reference details into Active claims; do not leave a duplicate filing-action card in the action queue.
