...
- Below the hero, add one compact pure-CSS status strip with only four decision-oriented links: **Action required**, **Purchase matches**, **Filed claims**, and **Security alerts**. `Action required` counts only unfiled 🟢/🟡 filing actions; `Purchase matches` counts the review cards; `Filed claims` counts all filed records, including the paid subset; and `Security alerts` counts 🔴 cards. Do not add Active, Watching, Paid, or Expired to this top strip; those remain available as labeled sections in the report body. Every strip entry must link to a real anchor and every count must match its underlying data. Keep the strip sticky on desktop so the four primary destinations remain available while scrolling, but make it static at 900px and below so it does not consume mobile space. Keep all four entries in a single row on mobile with short labels. Use these exact stable `id` targets: `action-queue`, `purchase-matches`, `active`, `watching`, `expired`, `filed`, `paid`, and `security`. Never substitute positional names such as `sec1`. Add `html { scroll-behavior: smooth; }` and use `scroll-margin-top` so anchored headings are not hidden.
